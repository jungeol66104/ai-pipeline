...
def simaqian():
    ...
    subject_batches = [subjects[i:i + 15] for i in range(0, len(subjects), 15)]
    raw_timelines = []
    for i, subject_batch in enumerate(subject_batches):
        print(f'\ttimeline extracting cycle #{i}')
        ...
        modify_storage_file_list('db/training_set', {"model": "simaqian", "input": json.dumps(input_data), "output": json.dumps(output_data)}, 'temporary.json')
        if output_data is not None:
            output_timelines = list(output_data.values())
            raw_timelines.extend(output_timelines)
