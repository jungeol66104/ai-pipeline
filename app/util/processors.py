...
def simaqian_processor(valid_raw_events, raw_timelines):
    ...
    for event in events_for_db:
        ...
        if timeline_from_db is not None:
            ...
            timelines_for_db.append({"id": timeline_id, "name": event["subject"], "description": next((raw_timeline["description"] for raw_timeline in raw_timelines if raw_timeline.get("name") == event["subject"]), None)})
            timeline_next_id += 1
        del event["subject"]
        # deal with event
